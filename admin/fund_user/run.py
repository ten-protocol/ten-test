...
class PySysTest(EthereumTest):
    ...
    def execute(self):
        ...
        web3, account = network.connect(Properties().funded_deployment_account_pk(self.env), network.HOST, network.PORT)
        ...
        with open(os.path.join(PROJECT.root, 'utils', 'contracts', 'erc20', 'erc20.json')) as f:
            poc_token = web3.eth.contract(address=Properties().l2_poc_token_address(self.env), abi=json.load(f))

        self.run_for_token(network, 'HOC', hoc_token, web3, account)
        self.run_for_token(network, 'POC', poc_token, web3, account)
    ...
    def run_for_token(self, network, token_name, token,
        ...
        for user_address in self.USERS:
            ...
            deploy_balance = token.functions.balanceOf(account.address).call()
            ...
            network.transact(self, web3, token.functions.transfer(user_address, self.AMOUNT), account, 7200000)
