...
class PySysTest(TenNetworkTest):
    ...
    def run_stop(self, network, web3, storage, account, value, duration, type):
        ...
        DockerHelper.container_stop(self, 'validator-enclave-0', time=duration, name=type)
        ...
        t1 = int(time.time())
        DockerHelper.container_start(self, 'validator-enclave-0', name=type)
        self.wait_for_validator()
        t2 = int(time.time())
        DockerHelper.container_logs(self, 'validator-enclave-0', state=BACKGROUND, name=type, since='%dm' % math.ceil((t2-t1) / 60))
