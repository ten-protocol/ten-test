...
class TenNetworkTest(GenericNetworkTest):
    ...
    def delete_session_key(self, url, address, return_error=False):
        ...
        params = {'sessionKeyAddress': address}
        data = {"jsonrpc": "2.0", "method": "eth_getStorageAt",
                "params": ["0x0000000000000000000000000000000000000004", json.dumps(params), None], "id": self.MSG_ID }
        response = self.post(data, url)
    ...
    def send_unsigned_against_session_key(self, url, address, base64_rlp):
        """Deactivate a session key. """
        params = {'sessionKeyAddress': address, 'tx': base64_rlp}
        data = {"jsonrpc": "2.0", "method": "eth_getStorageAt",
                "params": ["0x0000000000000000000000000000000000000005", json.dumps(params), None], "id": self.MSG_ID }
        response = self.post(data, url)
        if 'result' in response.json(): return response.json()['result']
        elif 'error' in response.json(): self.log.error(response.json()['error']['message'])
        return None
