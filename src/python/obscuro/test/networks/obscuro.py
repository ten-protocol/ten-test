    ...
    def connect(self, test, private_key, web_socket=False, check_funds=True):
        ...
        if check_funds :
            ...
            if balance < self.OBX_LIMIT:
                ...
                test.log.info('Account balance %.6f OBX below threshold %s' % (balance, self.OBX_LIMIT))
                test.fund_native(self, account, self.OBX_ALLOC, Properties().fundacntpk())
                test.log.info('Account balance %.6f OBX' % web3.fromWei(web3.eth.get_balance(account.address), 'ether'))
        return web3, account
