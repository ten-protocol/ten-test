...
class PySysTest(GenericNetworkTest):
    ...
    def execute(self):
        ...
        self.submit(account, error.contract.functions.set_key_with_revert("new"), web3, nonce, estimate)

        # force a require
        nonce = self.nonce_db.get_next_nonce(self, web3, account.address, self.env)
        self.submit(account, error.contract.functions.set_key_with_revert(""), web3, nonce, estimate, expect_success=False)
