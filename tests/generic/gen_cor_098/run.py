...
Item = namedtuple("Item", "gas value nonce")
...
class PySysTest(GenericNetworkTest):
    ...
    def execute(self):
        ...
        stack = Stack()
        stack.append(Item(int(0.9 * estimate_gas), 1, None))
        stack.append(Item(int(0.8 * estimate_gas), 2, None))
        stack.append(Item(int(0.6 * estimate_gas), 3, None))
        stack.append(Item(int(0.4 * estimate_gas), 4, None))
        stack.append(Item(int(0.1 * estimate_gas), 5, None))
        ...
        while not stack.isEmpty():
            ...
            if item.nonce is None: item = Item(item.gas, item.value, self.nonce(web3, account))
            ...
            try:
                ...
                stack.insert(Item(estimate_gas, item.value, item.nonce))
            except TransactionFailed as e:
                self.log.error(e)
                self.log.info('Inserting transaction to use next nonce but increase gas')
                stack.insert(Item(estimate_gas, item.value, None))
