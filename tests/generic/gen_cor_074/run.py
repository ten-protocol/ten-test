...
class PySysTest(GenericNetworkTest):
    ...
    def execute(self):
        ...
        self.waitForGrep(file=logout, expr='Started tasks', timeout=10)
        ...
        self.waitForGrep(file=logout, expr='Task1:', timeout=10)
        self.assertGrep(file=logout, expr='Task1: 10')

        # wait and validate - ItemSet2 filter on value 2 or 3
        # event ItemSet2(string key, uint256 indexed value)
        self.waitForGrep(file=logout, expr='Task2: k2 2', timeout=10)
        self.assertOrderedGrep(file=logout, exprList=['Task2: foo 2', 'Task2: bar 3', 'Task2: k2 2'])

        # validate correct count
        self.assertLineCount(file=logout, expr='Task2', condition='== 3')
