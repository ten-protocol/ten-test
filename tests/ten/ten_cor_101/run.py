...
class PySysTest(TenNetworkTest):
    ...
    def execute(self):
        ...
        self.log.info('  Balance of session key: %d' % web3.eth.get_balance(address))
        ...
        balance_after1 = web3.eth.get_balance(address)
        ...
        nonce = network.get_next_nonce(self, web3, address, persist_nonce=False)
        tx = network.build_transaction(self, web3, storage.contract.functions.store(3), nonce, address, storage.GAS_LIMIT)
        try:
        ...
        self.activate_session_key(network.connection_url())
        network.transact_unsigned(self, web3, storage.contract.functions.store(3), address, storage.GAS_LIMIT)
        call_value = storage.contract.functions.retrieve().call()
        balance_after2 = web3.eth.get_balance(address)
        self.log.info('  Call shows value: %d', call_value)
        ...
        network.tx_unsigned(self, web3, tx, address)
        balance_after3 = web3.eth.get_balance(address)
        ...
        balance_after4 = web3.eth.get_balance(address)
