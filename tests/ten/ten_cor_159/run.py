...
class PySysTest(TenNetworkTest):
    ...
    def execute(self):
        ...
        # check to ensure the total is the total rollups in the network, not the total in the return set
        self.log.info('')
        self.log.info('Doing a couple of calls on different pages sizes')
        r1 = self.scan_get_rollup_listing(offset=0, size=10)
        r2 = self.scan_get_rollup_listing(offset=0, size=20)
        ...
        # check to get all the rollups
        self.log.info('')
        self.log.info('Calling to iterate through all the rollups')
        offset = 0
        total = 0
        reported_total = 0
        numbers = []
        ...
        if len(duplicates) > 0:
            ...
            self.addOutcome(FAILED, 'There should be no duplicate rollup IDs in the iterated set')

        # check to see if we can read a large page
        if reported_total > 50:
            self.log.info('')
            try:
                self.scan_get_rollup_listing(offset=0, size=50)
