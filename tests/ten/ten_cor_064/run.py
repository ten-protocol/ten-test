...
from ten.test.contracts.game import FieldTopic2GuessGame
...
class PySysTest(TenNetworkTest):
    ...
    def execute(self):
        ...
        game_1 = FieldTopic2GuessGame(self, web3_1)
        ...
        game_2 = FieldTopic2GuessGame.clone(web3_2, account_2, game_1)
        ...
        self.assertLineCount('subscriber2.out', expr='Received event: Guessed', condition='==0')
        self.assertLineCount('subscriber2.out', expr='Received event: Attempts', condition='==4')
        self.assertGrep('subscriber1.out', expr='Received event: Guessed', contains=False)
        self.assertGrep('subscriber1.out', expr='Received event: Attempts', contains=False)
        ...
        for i in range(0, len(response)):
            ...
            self.assertTrue(response[i]['eventConfigPublic'] == False)    # Field.EVERYONE is not set
            self.assertTrue(response[i]['topic1Relevant'] == False)       # Field.TOPIC1 is not set
            self.assertTrue(response[i]['topic2Relevant'] == True)        # Field.TOPIC2 is set
            self.assertTrue(response[i]['topic3Relevant'] == False)       # Field.TOPIC3 is not set
            self.assertTrue(response[i]['senderRelevant'] == False)       # Field.SENDER is not set
