  ...
  contract.events.ItemSet3(
  {
    ...
    function(error, event){
       if (error) {
         ...
         fs.appendFile(options.event_log, log_time_ms.toString() + os.EOL, function (err) { if (err) throw err })
       }
  })
}
...
async function sendTransaction(key, value) {
  ...
  fs.appendFile(options.receipt_log, receipt_time_ms.toString() + os.EOL, function (err) { if (err) throw err })
